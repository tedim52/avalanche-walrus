...
func startTxSpamming() {
	...
	const SATURATED_RATE = 2.0
	const NUM_WORKERS = 500
	saturated := make(chan bool, 2)
...
// sampling from worker
func check_saturated(chan bool){
	// inquire fee
	// compare fee
	// if max, send signal

	// inquire block production rate 
	// compare rate
	// if mor than 2 
}